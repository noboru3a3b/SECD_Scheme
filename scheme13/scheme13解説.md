...
対象は `scheme13/scheme13.cpp`（単一ファイル、5,512 行）と
...
> **v1.14 で追ったもの**: **Windows（MinGW）版の起動を直した**
> （35日目の決定161・162）。二通りに壊れていた。**①`argv[0]` の区切りに
> `\` を見ていなかった** —— cmd / PowerShell から `.\scheme13\scheme13.exe`
> と起動すると探索候補（第10.7節）が cwd の2件に縮み、`lib13.scm` だけが
> 見つからなかった。**②GC のランタイム DLL がリポジトリのルートにしか
> 複製されていなかった** —— `cd scheme13` して起動すると `main` に入る前に
> `STATUS_DLL_NOT_FOUND` で死ぬ（何も出ない）。**どちらも「Windows で建つ」
> までしか想定していなかったことの帰結。** `--selftest` に
> `selftest_library_search`（4項目）を足した —— ファイルを読みに行かず
> 候補の並びを文字列で押さえるので、**円記号区切りの `argv[0]` を Linux の
> CI で試せる**。`selftest` 290 → **294**。
> **処理系の振る舞いは無変更**（`/` 区切りのときの候補は1バイトも変わらない）。
> **Windows 11 + w64devkit（MinGW）で確認済み** —— ルートからも `scheme13/` からも
> 起動する。FreeBSD / NetBSD は未確認のまま。
>
...
> **`(call/cc k)` は8行のうち唯一「行まるごと」空いていた行**で、
> `--selftest` の call/cc 6項目は受け手がどれも `lambda` だった。
> **「リーダの EOF 無限ループ」の1行の裏には5つの別々の実装があった。**
> 測り方そのものにも1つ分かったことがある: **同じ行でも変異の書き方で
> 結論が変わる**（記帳ごと外すと clang -O2 が畳む）。
> **処理系は無変更**。`make selftest` は 285 → 290。
>
> **v1.11 で追ったもの**: **設計憲章 §1.6（GC の規律）と §4.3（再帰の禁止事項）の
> 後ろ盾を測った**（31日目の決定148〜152）。11個の変異のうち **後ろ盾が
> あったのは2件だけ**で、しかもその2件は §4.3 のために置かれたものではなく
> 受け入れ基準 §5.2 から来ていた。**§5.2 の「ポートのディスクリプタ枯渇」の
> 行も、どのスイートも守っていなかった。** `tests/deep_test.scm` と
> `tests/fdlimit_test.scm` を新設し、`--selftest` に3項目足した（第12.9節）。
> **変異法でも測れないものが2種類ある**ことが分かったので、そこも書いた
> （最適化器が変異を消す `length`、出力に出ない §1.6 の 2・3・4）。
> **処理系は無変更**。`make selftest` は 282 → 285、`make test` は 41 → 43。
>
> **v1.10 で追ったもの**: **看板のデバッグ機能に後ろ盾を付けた**
> （30日目の決定145〜147）。測ってみると **§1.5 の7つのうち、出力に後ろ盾が
> あったのは `help` と `macro-print` の2つだけ**で、4件は `(help)` の文面に
> 名前が載っているだけ、`trace-on` に至っては `test_improvements.scm` が
> 名前を含む文字列を印字していただけだった。`tests/debug_test.scm` を新設し、
> **21個の命令をすべて出す**ように式を選んである（第12.2節）。
> **処理系は無変更**なので行数・個数は動いていない。`make test` は 40 → 41。
>
...
> 終了状態の3つを別々に**押さえる。第12.2節の件数（25 → 40）と、
> **新設した第12.7節**（`--selftest` が見ていなかったエラーの包み）と
> **第12.8節**（REPL）。**処理系の振る舞いは1つも変えていない。**
>
> **v1.6 で追ったもの**: **凍結仕様 §2 の後ろ盾を測った**（26日目の決定132〜134）。
> 実装を1点ずつ壊して既存テストが捕まえるかを見る変異法で 57箇所を測り、
> 後ろ盾がゼロの行が5つ見つかった。埋めるために
> **`tests/spec_test.scm` を新設**（§2 の各行を1行ずつ写す）し、
> `selftest_errors()` に `(/ 1 0)` を足した。テストの件数（第12章）と
> 互換性の証拠の範囲（第13.1・13.4節）、**24・25日目に増えた SICP の件数**
> （第12.5節。697 → 1050）。**処理系の振る舞いは1つも変えていない。**
>
...
**実行ファイルの位置は `argv[0]` から取るので、区切りは `/` と `\` の両方を
見る。** MinGW 版を cmd / PowerShell から `.\scheme13\scheme13.exe` と
起動すると `argv[0]` は円記号区切りで渡り、`/` だけを探すと 2. と 3. が
丸ごと落ちる —— リポジトリのルートから起動したとき、cwd に在る
`system_lib.scm` は見つかり、`scheme13/` に在る `lib13.scm` だけを失う、
という壊れ方をしていた（35日目の決定161）。回帰は `selftest_library_search`。

**Windows では、GC のランタイム DLL も実行ファイルの隣に要る**（決定162）。
ローダは `.exe` の在るディレクトリを最初に見るので、`scheme13/` に
`libgc-1.dll` と `libgccpp-1.dll` が無いと **`main` に入る前に**
`STATUS_DLL_NOT_FOUND` で死ぬ。**処理系がまだ動いていないので何も出ない。**
ルートの `Makefile` が両方に複製する。
...
**この節の出力は `tests/debug_test.scm` が固定している**（30日目の決定146）。
30日目まで、**`compile` / `disassemble` / `globals` / `macros` / `trace-on` の
出力を見ているテストは1つも無かった**（決定145）。`(help)` の文面に名前が
載っているので grep では見つかってしまうが、**名前が出力に現れることと、
その機能の出力が固定されていることは別である**。

`debug_test.scm` の第1節は **21個の命令をすべて出す**ように式が選んであり、
命令セットに手が入れば必ずどこかが動く。**§1.5 の「命令セットを変えるたびに
逆アセンブル表示を追随させること」を、人の目から機械へ移す仕掛けである。**
覆えているかを確かめるコマンドは `tests/golden/README.md` にある。

**`(globals)` だけは入れていない**（26日目の決定136）。線は
**「採り直しが何かを壊した合図になるか」**で引く（決定147）——
`globals` の採り直しは「名前を足した」以上のことを言わないが、
`disassemble` の採り直しは「命令セットに手を入れた」という §1.5 が
知りたい事実そのものである。**「追随が要るから入れない」ではない。**
...
```sh
make -C scheme13 selftest   # 294 checks — 凍結仕様との突き合わせ（C++ 単体）
make -C scheme13 compare    #  15 passed — 構文展開が scheme12 と等価か
make -C scheme13 test       #  43 passed — 既存 .scm 資産との互換性（受け入れ基準）
...
**この層は「補助」ではない。** 26日目に測って凍結仕様 §2 の11行を単独で
支えていると分かり（第12.6節）、32日目に測って**受け入れ基準 §5.2 の8行のうち
7行もここが単独で支えている**と分かった（第12.10節）。**ここが消えたら
代わりが無い。**
...
| `selftest_positions` | 位置の記録とキャレット行。**リーダが EOF に出会う5経路**（第12.10節） |
| `selftest_eval` | 評価（算術・クロージャ・末尾呼び出し・継続・fixnum 境界）。**継続を call/cc に渡せること**（第12.10節） |
...
| `selftest_library_search` | **起動時ライブラリの探索候補**（第10.7節）。円記号区切りの `argv[0]` を含む |

### 12.2 ゴールデン（19件）+ 起動時ライブラリ（3件）+ 終了コード（5件）
###      + エラーの包み（7件）+ REPL の筋道（5件）+ ポートの後始末（1件）
###      + コマンド行（2件）
...
| `scheme13/tests/spec_test.scm` | 同上（**凍結仕様 §2 の各行**。第12.6節） |
| `scheme13/tests/debug_test.scm` | 同上（**看板のデバッグ機能**。第11.3節。30日目） |
| `scheme13/tests/deep_test.scm` | 同上（**cdr 方向を再帰で辿らないこと**。第12.9節。31日目） |
| `scheme13/tests/fdlimit_test.scm` | 同上（**ポートの後始末**。第12.9節。31日目） |

加えて6種類の回帰が入っている（合計 43 件）。
...
- **エラーの包みが7件 + 順序が1件**（`tests/errors/*.scm`。第12.7節）。
  エラーで止まるので普通のゴールデンには書けない。**標準出力・標準エラー・
  終了状態を別々に**見て、1本だけ**両者を混ぜた順序**も見る
- **REPL の筋道が5件**（`tests/repl/*.in`。第12.8節）。`.in` を標準入力に
  流し込んで、同じく3つを別々に見る
- **ポートの後始末が1件**（`tests/fdlimit_test.scm`。第12.9節）。
  `ulimit -n 64` に下げて5000回オープンさせる。**上限を下げるのはシェルにしか
  できない**ので普通のゴールデンには置けず、下げられない環境では `SKIP` と出る
- **コマンド行の誤りが2件**（`--bogus` と、パスの無い `--load`）。
  どちらも stderr に出て終了状態 1
...
### 12.5 SICP を教材にした実地確認（1050項目）

```sh
scheme13/sicp/run_sicp.sh          # 25 passed, 0 failed（主張 1050 件）
```
...
| 第5章（5.1〜5.5） | 352 | **レジスタ機械と翻訳系。scheme13 の設計（第5〜8章）と直接重なる章**で、対応表は `sicp/README.md` にある。24日目に節の主題を216項目、25日目に演習・字句アドレス・3通りの比較を136項目 |
...
23日目に第1章から第4章まで、24・25日目に第5章を通し、**SICP は読み切った**。
第4・5章では処理系に1行も手を入れていない。**そこで見つかった不具合が
...
### 12.6 凍結仕様 §2 の後ろ盾（`tests/spec_test.scm`）

上の5つは**どれも §2 を目的に作られていない**。ゴールデンは「scheme12 と
1バイトも違わないこと」、SICP は「教科書の題材が動くこと」を見ている。
その結果、**§2 の行のうち誰も守っていないものがあった。**

26日目に**変異法**で測った（`dev_memo.md` の決定133）。実装を1点だけ壊して
§2 の1行を嘘にし、`selftest` / `compare` / `test` / SICP のどれが落ちるかを見る。
grep と違って「その経路が実際に走るか」まで込みで測れる。57箇所を測った結果:

| 測ったこと | 件数 |
| --- | --- |
| どれかが捕まえた | 52 |
| **どれも捕まえなかった** | **5** |
| うち `--selftest` だけが捕まえた（ゴールデンも SICP も見ていない） | 11 |
| SICP 1050項目が捕まえたもの | 6 |

後ろ盾がゼロだったのは
`(eq? 100000000000 100000000000)` が `TRUE`（§2.2）、
実数の `eqv?` がビット列比較（§2.2）、
2^53 を超える整数と実数の比較（§2.3）、
`(/ 1 0)` がエラー（§2.3）、
`(vector-set!` ではなく**`string-set!` の返り値が `:undef`**（§2.4）の5行。

**埋め方**。エラーで止まる `(/ 1 0)` だけは `selftest_errors()` に入れ、
残りは `tests/spec_test.scm` という1本にまとめた。この1本が
**§2 の各行を1行ずつ写している**。
...
左に §2 の項番と本文、右にその行が主張する値が並ぶので、
**ゴールデンの `.out` 自体が「§2 → 実際の振る舞い」の対応表として読める。**

原典のテスト機構（第12.3節）は**使っていない**。あちらは大小文字を無視して
比べるので §2.1「シンボルは大小文字を保存する」を守れず、
`#<closure:(x . r)>` のような**読み戻せない表現**も照合できない。
ここは `write` の出力そのものをゴールデンでバイト単位に押さえる。

**この方式で書けない §2 の行が3種類ある**（`spec_test.scm` の冒頭にも書いてある）。

| 書けないもの | どこが見ているか |
| --- | --- |
| エラーの文面（`(/ x)` / `(/ 1 0)` / 整数を要求する場所） | 途中で止まるのでゴールデンに書けない。`selftest_errors()` |
| 「無い」という主張（`syntax-rules` が無い・準クオートのネストが未対応） | 肯定形のテストで固定できない。踏むとエラーになる |
| フォームを跨ぐ継続の途中経過 | `spec_test.scm` の末尾で、出力の並びとして押さえている |

**§2 を次に改めるときは、この1本を同じ順序で直すこと。**

### 12.7 エラーの包み（`tests/errors/`）

**第12.2節までのゴールデンは、どれも正常に走りきるプログラムの出力である。**
既存 `.scm` 資産はエラーを1つも踏まない（7日目に分かったこと）。
だからエラーの回帰は `--selftest` だけが持っていた。

ところが `selftest_errors()` が見ているのは **`e.what()`、つまり本文だけ**
である（`eval_error_body()`）。`selftest_positions()` は `format_error()` を
キャレット行まで見るが、**リーダのエラーだけ**で、しかも架空のファイル名
（`t.scm`）を使う。**27日目まで、以下はどのテストも見ていなかった:**

| 見ていなかったもの |
| --- |
| `Fatal error: ` の見出し（`main` の `catch` が付ける。第3.2節） |
| `--load` に渡した**実際のパス**が出ること |
| **実行時**エラーのキャレット行（`selftest` はリーダのぶんだけ） |
| **終了状態が 1** であること |
| エラーが **stderr** に出て stdout には出ないこと |
| **stderr へ書く前に stdout を流す**こと |
| 読み取りエラーだと**1行も実行されない**こと |

`tests/errors/*.scm` の7本がこれを埋める。1本につきエラーは1つ
（Scheme レベルの例外捕捉が無いので処理系ごと止まる。第14.2節）。
`run_golden.sh` が**標準出力・標準エラー・終了状態の3つを別々に**見る。

```
Fatal error: scheme13/tests/errors/type_error.scm:6:1: car: wrong type of argument
  expected: a pair
  given: 5
    (car 5)
    ^
...
- `type_error.scm` — エラーより**前**の出力（`before the error`）は stdout に
  残り、**後ろ**は実行されない。さらに `.mix`（`> file 2>&1` で採った出力）で
  **両ストリームを混ぜたときの順序**を押さえる。`.out` と `.err` を別々に
  比べても順序は現れないので、`main` の `std::fflush(stdout)` を落としても
  気づけない — **足したテスト自体を変異法で確かめて見つかった穴**である（決定137）
- `reader_error.scm` — **標準出力が空**であることが主張である。
  `--load` は先にファイル全体を読むので、構文が壊れていればどこであっても
  **1行も実行されない**

`macro_position.scm` は、マクロが生んだ式のエラーが**利用者の書いた行**を
指すこと（決定52・53。第3.1節）を、包みごと押さえている。

### 12.8 REPL の筋道（`tests/repl/`）

**人が REPL を触る道筋は、27日目までどのテストも通っていなかった。**
`--selftest` もゴールデンも SICP も、**式を評価させること**しか見ていない。
15日目に `(exit)` が無いことへ14日間気づかなかったのも、根はここである
（第10.11節）。あのときは終了コード5件だけをシェル側で見るようにして、
残りは `dev_memo.md` §9 に積んであった。

`tests/repl/*.in` を標準入力に流し込み、やはり3つを別々に見る。

| ファイル | 主張 |
| --- | --- |
| `basic.in` | 起動の挨拶、`scheme13> ` プロンプト、値が返ること、EOF で `Bye!` と終了状態 0 |
| `error_survives.in` | **エラーを打っても REPL が死なない。** 文面は stderr、続きの式は評価される |
| `multiline.in` | 括弧が閉じるまで `...>` で読み続ける。**空行は読み直すだけ** |
| `exit_code.in` | REPL からの `(exit 3)`。`Bye!` は**出ない**（EOF ではない） |
| `help.in` | `(help)` の出力全体。**デバッグ機能は一級市民**（第11章）なので、文面が動いたらここで気づく |

**プロンプトは tty かどうかを見ずに必ず出る**ので、パイプで流し込んでも
出力が決定的になる。この測り方は実装のその性質に寄りかかっているので、
**プロンプトを tty のときだけ出すように変えたら、ここは全部採り直しになる。**

`(globals)` は入れていない。大域名が増えるたびにゴールデンが動くうえ、
名前の集合は第10.1節のやり方で機械的に数えるほうが確かだからである。

### 12.9 設計憲章 §1.6・§4.3 の後ろ盾（`tests/deep_test.scm` / `tests/fdlimit_test.scm`）

**憲章に書いてあることと、それを守る仕掛けがあることは別である。**
26日目に凍結仕様 §2 を変異法で測って後ろ盾ゼロの行を5つ見つけ（第12.6節）、
30日目に §1.5（看板のデバッグ機能）を測って7つのうち5つに後ろ盾が無いと
分かった（第11.3節）。**31日目は §1.6（GC の規律）と §4.3（再帰の禁止事項）を
同じやり方で測った。**

11個の変異を入れ、`--selftest` / `compare` / `test` / SICP のどれかが落ちるかを見た。

| 変異（実装を1点だけ壊す） | 憲章の行 | 30日目までの後ろ盾 |
| --- | --- | --- |
| `write_list` を cdr 方向の再帰にする | §4.3-1 | **なし** |
| `equal_values` を cdr 方向の再帰にする | §4.3-1 | あり（`--selftest` の `long equal?`） |
| `length` を再帰にする | §4.3-1 | **測れない**（下記） |
...
| `GC_invoke_finalizers()` だけ落とす | §1.6-5 | **測れない**（下記） |

**9件のうち後ろ盾があったのは2件だけだった。** しかもその2件は §4.3 を
目的に置かれたものではなく、受け入れ基準 §5.2（scheme12 で直した不具合の
再発防止）から来ている。**§5.2 には「ポートのディスクリプタ枯渇 — `ulimit -n 64`
で5000回オープン → 完走」という行もあるが、それを守るテストは無かった。**

**埋め方**は3つに分けた。

| 埋めたもの | どこへ |
| --- | --- |
| 深いリスト（`write` / `append` ほか cdr 方向を辿る手続き） | `tests/deep_test.scm`（新設。18項目） |
| 共有構造のバックトラック（`write` のベクタ、`equal?` のリストとベクタ） | `--selftest` の `shared DAG` の隣に3項目 |
| ポートの後始末（ファイナライザと EMFILE のやり直し） | `tests/fdlimit_test.scm`（新設）を `run_golden.sh` が `ulimit` を下げて走らせる |

#### 測れなかった2件

**`length` を再帰にしても、clang -O2 が畳んでループに戻す。**
`1 + length_rec(cdr)` は末尾呼び出しではないが、加算の再帰は蓄積変数へ
書き換えられる。`-O0` でビルドすれば300万要素で溢れるので**変異は本物**だが、
このプロジェクトのビルド設定（`-O2`）では基準と1バイトも違わない。
**つまり `length` を守っているのはテストではなく最適化器である。**
`deep_test.scm` に `(length big)` を並べてあるのは、最適化に頼らない形を
保つためで、いまの設定では何も測っていない。

**`GC_invoke_finalizers()` を落としても振る舞いが変わらない。**
Boehm GC は確保のたびに積まれたファイナライザを呼ぶので、`GC_gcollect()` だけでも
ディスクリプタは戻る。あの1行は `GC_set_finalize_on_demand(1)` にした場合への
備えであって、いまの設定では観測できない。**残す理由は §1.6-5 が明文で
求めているからで、テストで固定はできない。**

#### 測れない §1.6 の3行

§1.6 の 2（GC オブジェクトの文字列は `GcString`）・3（アロケータの使い分け）・
4（`cpp_int` の limb は ptrfree）は、**壊しても出力が1バイトも変わらない。**
現れるのはメモリの漏れと偽 retention だけで、どちらも「どれだけ増えたら不合格か」
を決められない。**この3行はテストでは固定できない**（26日目に §2 の
「〜が無い」という主張が肯定形のテストで書けないと分かったのと同じ形である）。

#### 次に触る人へ

- **`deep_test.scm` の 100万という数には根拠がある。** 既定のスタック 8MB のとき、
  `append` を素朴な再帰で書くと25万〜30万の間で溢れる。3倍強の余裕を取ってあり、
  スタックが 24MB ある機械でも溢れる。**小さくすると静かに何も測らなくなる**
...
- **`fdlimit_test.scm` は単体で走らせても何も測らない。** ディスクリプタが
  余っていれば5000回など何事もなく開ける。`ulimit` を下げられない環境では
  `run_golden.sh` が `SKIP` と出す（**`PASS` ではない**）

### 12.10 受け入れ基準 §5.2 の後ろ盾（32日目）

**「確認方法」の欄に手順が書いてあることと、機械が走らせていることは別である。**
`dev_memo.md` §5.2 は「scheme12 で修正済みの不具合が再発しないこと」を8行の表に
していて、各行に**人がやる確認手順**が添えてある。31日目にそのうち1行
（ポートのディスクリプタ枯渇）をどのスイートも守っていないと分かったので、
**32日目に残る7行も同じやり方で測った。**

**1行が1つの実装とは限らない。** とくに「リーダの EOF 無限ループ」の1行の裏には
**5つの別々の実装**があり、守られていたのは2つだけだった。8行を14の変異で測った。

| 変異（実装を1点だけ壊す） | §5.2 の行 | 31日目までの後ろ盾 |
| --- | --- | --- |
| `read_list` のループ先頭の EOF 検査を `break` にする | EOF | あり |
| ドット尻尾を読んだ後の EOF 検査を `break` にする | EOF | **なし** |
| `read_vector` の EOF 検査を `break` にする | EOF | **なし** |
| `read_string` のループ先頭の EOF 検査を `break` にする | EOF | あり |
| `read_abbrev` の EOF を `g_nil` で埋める | EOF | **なし** |
| `write_list` の car 再帰に新しい `PathSet` を渡す | 循環表示 | あり |
| `write_vector` の要素再帰に新しい `PathSet` を渡す | 循環表示 | あり |
| `equal_values` を cdr 方向の再帰にする（記帳を残す形） | 長い `equal?` | あり |
| 同上（記帳ごと外した形） | 長い `equal?` | **等価変異**（下記） |
| `scan_out_defines` を素通りさせる | 内部 define | あり（**ゴールデンは捕まえない**） |
| `qq_transfer` のドット位置の `unquote` を拾わない | 準クオート | あり |
| `qq_transfer` のベクタの枝を通らなくする | ベクタ内 unquote | あり |
| `CALLCC` が継続を受け手として拒む | `(call/cc k)` | **なし** |

**`(call/cc k)` は8行のうち唯一「行まるごと」後ろ盾が無かった。**
`--selftest` には call/cc の項目が6つあるが、**受け手はどれも `lambda`** で、
継続そのものを渡す形はどのスイートも一度も通っていなかった。

**埋め方は4件とも `--selftest`。** 置き場所は「同じ性質が既にどこに住んでいるか」
で決めた。リーダの EOF は `selftest_positions()` に2件が既にあるのでその隣へ3件、
`(call/cc k)` は `selftest_eval` の call/cc の並びへ2件。**新しいファイルは
作っていない** — §5.2 の8行は性質がばらばらで、1本にまとめようとしても
必ず散らばる（エラーで止まる行・`ulimit` が要る行・100万要素の行がある）。
代わりに `dev_memo.md` §5.2 の表に**3列目（後ろ盾）**を足し、散らばった置き場所を
文書の側が指すようにした。**§5.2 に行を足したら、3列目も同時に埋めること。**

#### 変異の書き方で結論が変わる

「長いリストの `equal?`」を最初に測ったとき **「後ろ盾なし」と出た**。31日目の
表は同じ行を「あり」と記録しているので、どちらかが誤りである。調べたら
**変異の書き方の違い**だった。

| 変異の形 | `-O2` での挙動 | 測定結果 |
| --- | --- | --- |
| `return equal(car) && equal(cdr);` | 20万でも300万でも**溢れない** | 後ろ盾なし（誤り） |
| **`seen` の記帳を残したまま再帰にした** | 20万で**溢れる** | あり |

`seen` の出し入れごと外すと clang -O2 が再帰をループへ畳んでしまう
（`-O0` でビルドすれば1つ目でも溢れるので、変異そのものは本物である）。
**変異は「機能を1つ抜く」形にし、まわりの記帳は残すこと。** 抜きすぎると、
壊したつもりの経路ごと最適化器が作り直す。第12.9節の `length` は
「測れない」話だったが、こちらは**「測ったつもりで逆の答えが出る」**ので
性質が悪い。**「後ろ盾なし」と出たら、まずその変異が本当に振る舞いを
変えているかを確かめること。**

#### ゴールデンは §5.2 を守っていない

`scan_out_defines` を素通りさせる（内部 define が大域定義になる）と
`--selftest` と SICP は落ちるが、**`make test` は 43件すべて通る。**
`tests/spec_test.scm` には §2.6 の項目が7つあるのに、である。

理由は、あの7項目が主張しているのが**「大域へ漏れるほうの3ケース」**だからで、
素通り変異はそちら側を1つも変えない。**letrec になるほうを主張しているのは
2.6-01 だけ**で、それも `(+ a b)` が 3 になることしか見ておらず、
大域定義でも 3 になる。**§5.2 が名指ししている名前衝突ケース（`(10 15 10)`）は
どのゴールデンにも書かれていない**（変異を入れると `(15 15 15)` になる）。

**§5.2 を守っているのは、ほぼ `--selftest` 一本である。** 8行中7行を
`--selftest` が捕まえ、**ゴールデンが単独で守っている行はゼロ**だった。
第12.6節に「`--selftest` が思っていたより重い」と書いたが、§5.2 については
もっと極端で、**ここが消えたら受け入れ基準 §5.2 のほぼ全部が同時に消える。**
...
| 凍結仕様 §2 | 285項目 + `spec_test.scm` で固定 | `make selftest` / `make test` |
...
影響範囲: `case` は `memv` に展開されるので無影響。ゴールデン（当時17件）にも無影響。
...
一致の根拠は**リポジトリにある 12 個の `.scm` 資産と selftest 294 項目**で、
