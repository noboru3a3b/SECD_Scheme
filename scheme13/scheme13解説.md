...
| `LDF tmpl` | 現在の環境を捕まえてクロージャを作り、積む |
...
make -C scheme13 test       #  16 passed — 既存 .scm 資産との互換性（受け入れ基準）
...
一致の根拠は**リポジトリにある 12 個の `.scm` 資産と selftest 169 項目**で、
...
いまの実装で 27 倍出ているので急がないが、もし足りなくなったら順に:

1. `if` を `SEL`/`JOIN` からジャンプ命令へ
2. マクロ展開のメモ化（原典がやっている）
3. `LD` の環境たどりを、深さ0の場合に特化する
