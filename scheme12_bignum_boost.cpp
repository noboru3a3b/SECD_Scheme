...
// Use Boost.Multiprecision if available, otherwise fall back to long long
#if __has_include(<boost/multiprecision/cpp_int.hpp>)
#include <boost/multiprecision/cpp_int.hpp>
#define HAS_BIGNUM
using BigIntType = boost::multiprecision::cpp_int;
#else
// Fallback to long long (limited precision)
...
struct BigInt : public gc {
    ...
    explicit BigInt(const std::string& s, int base = 10) {
        (void)base; // unused in fallback mode
#ifdef HAS_BIGNUM
        value = BigIntType(s);
#else
        value = std::stoll(s);
...
struct Closure : public gc {
    ...
    Closure(std::vector<std::string, gc_allocator<std::string>> p, std::optional<std::string> r, CodePtr b, ValueVec env)
...
static ValuePtr make_value(Value::Data d) {
    return new Value(std::move(d));
}

static CodePtr make_code() {
    return new Code();
}
...
//static ValuePtr make_int(long long n) {
//    return make_int(BigInt(n));
//}
...
static ValuePtr make_cont(const ValueVec& s, const ValueVec& e, CodePtr c, std::size_t pc, const DumpFrameVec& d) {
...
//static bool is_int(const ValuePtr& v) {
//    return v && std::holds_alternative<BigIntPtr>(v->data);
//}
...
static std::string to_string(const ValuePtr& v) {
    ...
    if (std::holds_alternative<BigIntPtr>(v->data)) return std::get<BigIntPtr>(v->data)->to_string();
    if (std::holds_alternative<std::string>(v->data)) return "\"" + std::get<std::string>(v->data) + "\"";
    if (std::holds_alternative<Symbol>(v->data)) return std::get<Symbol>(v->data).name;
    if (std::holds_alternative<PairPtr>(v->data)) return list_to_string(v);
    ...
    ValuePtr read_list() {
        ...
        while (true) {
            ...
            if (peek() == '.') {
                ...
                ValuePtr out = tail;
                for (auto it = items.rbegin(); it != items.rend(); ++it) out = make_pair(*it, out);
                return out;
            }
            items.push_back(read_expr());
        }
...
static std::optional<std::pair<int, int>> location_of(const std::string& name, const CompileEnv& env) {
...
static ValuePtr expand_let(ValuePtr rest) {
    ...
    if (is_symbol(bindings)) {
        ...
        ValuePtr lambda_expr = make_pair(make_symbol("lambda"), make_pair(list_from_vector(params), real_body));
        ...
        return list_from_vector({make_symbol("letrec"), list_from_vector({rec_bind}), list_from_vector(call_items)});
    }
    ...
    ValuePtr lambda_expr = make_pair(make_symbol("lambda"), make_pair(list_from_vector(params), body));
    ValueVec call_items;
...
static ValuePtr expand_let_star(ValuePtr rest) {
    ...
    if (bs.size() == 1) return list_from_vector({make_symbol("let"), list_from_vector({bs[0]}), make_begin_form(vector_from_list(body))});
    ValuePtr first = bs[0];
    ValueVec tail_bindings(bs.begin() + 1, bs.end());
    ValuePtr nested = list_from_vector({make_symbol("let*"), list_from_vector(tail_bindings), make_begin_form(vector_from_list(body))});
...
static ValuePtr expand_letrec(ValuePtr rest) {
    ...
    return list_from_vector({make_symbol("let"), list_from_vector(let_bindings), make_begin_form(seq)});
}
...
static ValuePtr expand_and(ValuePtr rest) {
    ...
    return list_from_vector({make_symbol("if"), xs[0], expand_and(list_from_vector(tail)), make_bool(false)});
}
...
static ValuePtr expand_or(ValuePtr rest) {
    ...
    ValuePtr body = list_from_vector({make_symbol("if"), t, t, expand_or(list_from_vector(tail))});
...
static ValuePtr expand_cond(ValuePtr rest) {
    ...
    return list_from_vector({make_symbol("if"), test, make_begin_form(vector_from_list(body)), expand_cond(rem)});
}
    ...
    if (is_pair(ls)) {
        ...
        if (is_pair(a)) {
            if (is_symbol(car(a), "unquote")) {
                return list_from_vector({make_symbol("cons"), car(cdr(a)), qq_transfer(cdr(ls))});
            }
            if (is_symbol(car(a), "splice")) {
                return list_from_vector({make_symbol("append"), car(cdr(a)), qq_transfer(cdr(ls))});
            }
            return list_from_vector({make_symbol("cons"), qq_transfer(a), qq_transfer(cdr(ls))});
        }
        return list_from_vector({make_symbol("cons"), list_from_vector({make_symbol("quote"), a}), qq_transfer(cdr(ls))});
    }
...
static ValuePtr expand_do(ValuePtr rest) {
    ...
    ValuePtr lambda_expr = list_from_vector({make_symbol("lambda"), list_from_vector(vars), if_expr});
...
static bool extract_params(ValuePtr params_expr, std::vector<std::string, gc_allocator<std::string>>& fixed, 
...
static void comp(ValuePtr expr, const CompileEnv& env, CodePtr code, bool tail) {
    ...
    if (is_symbol(head, "lambda")) {
        ...
        std::optional<std::string> rest_param;
        if (!extract_params(params_expr, fixed, rest_param)) vm_error("invalid lambda params");
        CompileEnv env2 = env;
    ...
    if (is_symbol(head, "define")) {
        ...
        if (is_pair(lhs) && is_symbol(car(lhs))) {
            ...
            ValuePtr lambda_expr = make_pair(make_symbol("lambda"), make_pair(params, rhs_tail));
...
// VM (省略なしで�?のscheme12.cppと同じ実�?を使用)
struct VM {
    ValueVec s;
    ValueVec e;
    CodePtr c;
    ...
    ValuePtr run() {
        while (true) {
            ...
            switch (ins.op) {
                ...
                case Op::LDF:
                    s.push_back(make_closure(ins.lambda_params, ins.lambda_rest, ins.lambda_code, e));
                    break;
                case Op::ARGS: {
                    if (ins.a < 0 || static_cast<std::size_t>(ins.a) > s.size()) vm_error("ARGS stack underflow");
                    ValueVec v;
                ...
                case Op::ARGS_AP: {
                    ...
                    if (static_cast<std::size_t>(ins.a) > s.size()) vm_error("ARGS-AP stack underflow");
                ...
                case Op::TAPP: {
                    ...
                    s.pop_back();
                    if (std::holds_alternative<PrimitiveFn>(callee->data)) {
                        auto args = vector_from_list(lvar);
                        ValuePtr out = std::get<PrimitiveFn>(callee->data)(args);
                    ...
                    if (!std::holds_alternative<ClosurePtr>(callee->data)) vm_error("APP target is not callable");
                    ClosurePtr clo = std::get<ClosurePtr>(callee->data);
                    auto args = vector_from_list(lvar);
                    ValueVec frame = args;
                    if (!clo->rest_param) {
                        if (args.size() != clo->params.size()) vm_error("arity mismatch");
                    } else {
                        if (args.size() < clo->params.size()) vm_error("arity mismatch (rest)");
                    ...
                    new_e.insert(new_e.end(), clo->captured_env.begin(), clo->captured_env.end());
                    e = new_e;
                    c = clo->body;
                    pc = 0;
                    break;
                ...
                case Op::CALLCC: {
                    ...
                    if (std::holds_alternative<PrimitiveFn>(proc->data)) {
                        auto args = vector_from_list(lvar);
                        ValuePtr out = std::get<PrimitiveFn>(proc->data)(args);
                        s.push_back(out ? out : g_nil);
                        break;
                    }
                    if (!std::holds_alternative<ClosurePtr>(proc->data)) vm_error("CALLCC target is not callable");
                    ClosurePtr clo = std::get<ClosurePtr>(proc->data);
                    ValueVec args = vector_from_list(lvar);
                    ValueVec frame = args;
                    if (!clo->rest_param) {
                        if (args.size() != clo->params.size()) vm_error("arity mismatch in CALLCC");
                    } else {
                        if (args.size() < clo->params.size()) vm_error("arity mismatch (rest) in CALLCC");
                        ValueVec fixed(args.begin(), args.begin() + static_cast<long>(clo->params.size()));
                        ValueVec rest(args.begin() + static_cast<long>(clo->params.size()), args.end());
                        frame = fixed;
                        frame.push_back(list_from_vector(rest));
...
static ValuePtr prim_set_car(const ValueVec& args) {
    if (args.size() != 2 || !is_pair(args[0])) vm_error("set-car! expects pair and value");
    as_pair(args[0])->car = args[1];
    return args[1];
}

static ValuePtr prim_set_cdr(const ValueVec& args) {
    if (args.size() != 2 || !is_pair(args[0])) vm_error("set-cdr! expects pair and value");
...
static ValuePtr prim_eq(const ValueVec& args) {
    ...
    if (std::holds_alternative<BigIntPtr>(args[0]->data) && std::holds_alternative<BigIntPtr>(args[1]->data)) {
...
static void load_startup_libraries(const char* argv0) {
    ...
    if (argv0 && *argv0) {
        std::error_code ec;
        std::filesystem::path exe_path = std::filesystem::absolute(std::filesystem::path(argv0), ec);
        if (!ec) {
...
static void init_globals() {
    ...
    g_globals["car"] = make_prim(prim_car);
    g_globals["cdr"] = make_prim(prim_cdr);
    g_globals["caar"] = make_prim(prim_caar);
    g_globals["cdar"] = make_prim(prim_cdar);
    g_globals["cadr"] = make_prim(prim_cadr);
    g_globals["cddr"] = make_prim(prim_cddr);
    g_globals["caddr"] = make_prim(prim_caddr);
...
#else
                    std::cout << "scheme12 (SECD with long long - limited precision)\n";
#endif
                    std::cout << "Usage:\n";
                    std::cout << "  scheme12              Start interactive REPL\n";
                    std::cout << "  scheme12 --load FILE  Evaluate file and exit\n";
                    std::cout << "  scheme12 --help       Show this help\n";
                    return 0;
                }
                if (arg == "--load") {
                    if (i + 1 >= argc) {
